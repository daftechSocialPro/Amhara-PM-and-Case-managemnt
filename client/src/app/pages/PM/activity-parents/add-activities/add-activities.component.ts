...
export class AddActivitiesComponent implements OnInit {
  ...
  // getValidatorForGoal(){
  //   this.getUnitOfMesurmentType()
  //   if(this.activityForm.value.UnitOfMeasurement.)
  // }
  unitOfMeasurementType(event: any){
    
    this.mesurmentType = this.unitMeasurments.find(x => x.Id === this.activityForm.value.UnitOfMeasurement)?.Photo
    const unitOfMeasurementControl = this.activityForm.get('Goal');
    if (this.mesurmentType === "percent" && unitOfMeasurementControl) {
        unitOfMeasurementControl.setValidators([Validators.required, Validators.max(100), Validators.min(0)]);
    } else if (unitOfMeasurementControl) {
        unitOfMeasurementControl.setValidators(Validators.required);
    }
    unitOfMeasurementControl?.updateValueAndValidity();
  }
  ...
  weightChange(weight:string){
    let weightt: number = Number(weight); 
    if(this.activity){
      weightt += Number(this.activity.Weight);
    }
    if (this.task){
      if ( weightt >this.task.RemianingWeight){
