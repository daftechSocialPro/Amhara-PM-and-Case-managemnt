    ...
    public class CaseProccessingService : ICaseProccessingService
    {
        ...
        public async Task<int> ConfirmTranasaction(ConfirmTranscationDto confirmTranscationDto)
        {
            ...
            try
            {
                ...
                CaseHistory? history = _dbContext.CaseHistories.Find(confirmTranscationDto.CaseHistoryId);
        ...
        public async Task<int> AssignTask(CaseAssignDto caseAssignDto)
        {
            try
            {
             ...
             _dbContext.Employees.FirstOrDefault(
                ...
                var childCasesTypes = await _dbContext.CaseTypes.Where(x => x.ParentCaseTypeId == caseToAssign.CaseTypeId).OrderBy(x => x.OrderNumber).Select(x => x.Id).ToListAsync();

                CaseHistory startupHistory = new CaseHistory
                {
                    Id = Guid.NewGuid(),
                    ...
                    //ChildCaseTypeId = childCasesTypes[0],
                ...
                if (childCasesTypes.Count > 0)
                {
                    startupHistory.ChildCaseTypeId = childCasesTypes[0];
                }
                ...
                {
                    foreach (Guid row in caseAssignDto.ForwardedToStructureId)
                    {

                        Guid toEmployee = new Guid();
                ...
                List<CaseEncodeGetDto> assigndCase = await _caseEncodeService.GetAllTransfred(toEmployeeId.Value);
        ...
        public async Task<int> CompleteTask(CaseCompleteDto caseCompleteDto)
        {
            try
            {
                ...
                List<CaseHistory> selectedHistoryCCs = _dbContext.CaseHistories.Where(x => x.CaseId == selectedHistory.CaseId && x.ReciverType == ReciverType.Cc).ToList();
                ...
                if (selectedHistoryCCs != null)
                {
                    foreach (CaseHistory selectedHistoryCC in selectedHistoryCCs)
                    {
                ...
                Case? selectedCase = _dbContext.Cases.Find(selectedHistory.CaseId);
                selectedCase.CompletedAt = DateTime.Now;
                selectedCase.AffairStatus = AffairStatus.Completed;
                selectedCase.CompletedOrgStructureId = caseCompleteDto.OrganizationStructureId;
                ...
                _dbContext.Entry(selectedCase).Property(x => x.CompletedOrgStructureId).IsModified = true;

                await _dbContext.CaseAttachments.AddRangeAsync(caseCompleteDto.CaseAttachments);

                await _dbContext.SaveChangesAsync();

                List<Employee> employees = await _dbContext.Employees.Include(x => x.OrganizationalStructure).Where(x => x.Id == selectedHistory.ToEmployeeId).ToListAsync();
                if (employees.Any())

                {
                    Employee? employee = employees.FirstOrDefault();

                    List<Activity> activities = await _dbContext.Activities.Where(x => x.CaseTypeId == currentCase.CaseTypeId && x.OrganizationalStructureId == employee.OrganizationalStructure.OrganizationBranchId).ToListAsync();

                    if (activities.Any())
                    {
                        Activity? activity = activities.FirstOrDefault();
                        List<ActivityTargetDivision> actTarget = await _dbContext.ActivityTargetDivisions.Where(x => x.ActivityId == activity.Id).ToListAsync();
                        if (activity != null && actTarget.Any())
                        {

                            ActivityProgress activityProgress2 = new ActivityProgress
                            {
                                Id = Guid.NewGuid(),
                                CreatedAt = DateTime.Now,
        ...
        public async Task<int> RevertTask(CaseRevertDto revertCase)
        {
            try
            {
                ...
                CaseHistory newHistory = new()
                {
                    ...
                    ChildCaseTypeId = selectedHistory.ChildCaseTypeId,
                    ReciverType = ReciverType.Orginal,
                    childOrder = selectedHistory.childOrder != 0 ? selectedHistory.childOrder - 1 : selectedHistory.childOrder,
                };
        ...
        public async Task<int> TransferCase(CaseTransferDto caseTransferDto)
        {
            try
            {
                ...
                CaseHistory currentLastHistory = await _dbContext.CaseHistories.Where(el => el.Id.Equals(caseTransferDto.CaseHistoryId)).Include(x => x.Case).ThenInclude(x => x.CaseType).OrderByDescending(x => x.CreatedAt).FirstAsync();
                ...
                CaseHistory newHistory = new CaseHistory
                {
                    ...
                    //ChildCaseTypeId = childCasesTypes[currentLastHistory.childOrder + 1],
                    childOrder = currentLastHistory.childOrder + 1
                    //must be change
                };
                if (childCasesTypes.Count > 0)
                {
                    newHistory.ChildCaseTypeId = childCasesTypes[currentLastHistory.childOrder + 1];
                }
                ...
                List<CaseEncodeGetDto> assigndCase = await _caseEncodeService.GetAllTransfred(toEmployee);
        ...
        public async Task<ResponseMessage> AddToWaiting(Guid caseHistoryId)
        {
            try
            {
                ...
                CaseHistory? history = _dbContext.CaseHistories.Find(caseHistoryId);
        ...
        public async Task<int> SendSMS(CaseCompleteDto smsdetail)
        {
            ...
            CaseHistory? history = _dbContext.CaseHistories.Find(smsdetail.CaseHistoryId);
        ...
        public async Task<int> ArchiveCase(ArchivedCaseDto archivedCaseDto)
        {
            ...
            cases.ArchivedTime = DateTime.Now;
        ...
        public async Task<CaseState> GetCaseState(Guid CaseTypeId, Guid caseHistoryId)
        {
            ...
            List<CaseType> childCaseType = await _dbContext.CaseTypes.Where(x => x.ParentCaseTypeId == CaseTypeId).ToListAsync();
            CaseState caseState = new CaseState();

            int childcount = _dbContext.CaseHistories.Find(caseHistoryId).childOrder + 1;
            foreach (CaseType? childaffair in childCaseType)
            {
                ...
                if (childaffair.OrderNumber == childcount)
                {
                    ...
                    List<FileSetting> files = await _dbContext.FileSettings.Where(x => x.CaseTypeId == childaffair.Id).ToListAsync();

                    foreach (FileSetting? file in files)
                    {
        ...
        {
            Guid caseIDD = _dbContext.CaseHistories.Find(caseId).CaseId;
            Guid? employee = _dbContext.CaseHistories.Where(x => x.CaseId == caseIDD && x.ReciverType == ReciverType.Orginal).OrderByDescending(z => z.CreatedAt).FirstOrDefault().ToEmployeeId;

            List<DateTime?> casehistor = _dbContext.CaseHistories.Where(x => x.CaseId == caseIDD).OrderBy(x => x.childOrder).Where(x => x.CompletedDateTime != null).Select(x => x.CompletedDateTime).ToList();
