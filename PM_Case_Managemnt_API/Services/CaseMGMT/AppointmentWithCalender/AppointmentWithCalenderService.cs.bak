    ...
    public class AppointmentWithCalenderService : IAppointmentWithCalenderService
    {
        ...
        public async Task<AppointmentGetDto> Add(AppointmentWithCalenderPostDto appointmentWithCalender)
        {
            try
            {
                ...
                Case cases = _dbContext.Cases.Include(x => x.Applicant).Include(x => x.Employee).Where(x => x.Id == hist.CaseId).FirstOrDefault();

                if (cases.Applicant != null)
                {
                ...
                {
                    ev.description = "Appointment with " + cases.Employee.FullName + " at " + appointment.Time + "\n Affair Number " + cases.CaseNumber;
        ...
        public async Task<List<AppointmentGetDto>> GetAll(Guid employeeId)
        {
            try
            {
                ...
                appointements.ForEach(a =>
                {
                    ...
                    ev.id = a.Id.ToString();
                    ev.description = "Appointment with " + (a?.Case?.Applicant?.ApplicantName ?? a?.Case?.Employee?.FullName) + " at " + a.Time + "\n Affair Number " + a.Case.CaseNumber;
