    ...
    public class CaserReportService : ICaseReportService
    {
        ...
        public async Task<CaseReportChartDto> GetCasePieChart(Guid subOrgId, string? startAt, string? endAt)
        {
            ...
            var report2 = (from q in report
                           ...
                           select new { q.Id, q.CaseTypeTitle }).Distinct();
            ...
            foreach (var eachreport in report2)
            {
                ...
                var allAffairs = _dbContext.Cases.Where(x => x.CaseTypeId == eachreport.Id);
                ...
                var caseTypeTitle = allAffairs.Select(x => x.CaseType.CaseTypeTitle).FirstOrDefault();

                if (caseTypeTitle != null)
                {
                    Chart.labels.Add(caseTypeTitle);
                }
        ...
        public async Task<CaseReportChartDto> GetCasePieCharByCaseStatus(Guid subOrgId, string? startAt, string? endAt)
        {
            ...
            Chart.labels = new List<string>() { "Assigned", "Completed", "Encoded" };
            Chart.datasets = new List<DataSets>();
        ...
        public double ConvertToHours(float counter, TimeMeasurement measurement)
        {
            switch (measurement)
            {
                ...
                case TimeMeasurement.Day:
                    return counter * 24;
                default:
                    throw new ArgumentOutOfRangeException(nameof(measurement), "Unknown time measurement type.");
            }
        }
        ...
        public async Task<List<EmployeePerformance>> GetCaseEmployeePerformace(Guid subOrgId, string key, string OrganizationName)
        {
            ...
            {
                var AffairHistories = _dbContext.CaseHistories.Include(x => x.CaseType).Include(x => x.Case.CaseType).Include(x => x.ChildCaseType).Where(ah =>
                      ah.ToEmployeeId == employee.Id).ToList();
                var actualTimeTaken = 0.0;
                var expectedTime = 0.0;
                var expectedTimeInHours = 0.0;
                ...
                if (AffairHistories != null)
                {
                    AffairHistories.ForEach(history =>
                    {
                        ...
                        double counterInHours;


                        var counter = history.ChildCaseType?.Counter ?? history.CaseType?.Counter ?? history.Case.CaseType.Counter;
                        var measurement = history.ChildCaseType?.MeasurementUnit ?? history.CaseType?.MeasurementUnit ?? history.Case.CaseType.MeasurementUnit;
                        counterInHours = ConvertToHours(counter, measurement);

                        expectedTime += counterInHours;
                    });
                }
        ...
        public async Task<List<CaseDetailReportDto>> GetCaseDetail(Guid subOrgId, string key)
        {
            ...
            affairs.OrderByDescending(x => x.Createdat).ToList().ForEach(af =>
            {
            ...
            var result2 = affairs.OrderByDescending(x => x.Createdat).ToList();
        ...
        public async Task<CaseProgressReportDto> GetCaseProgress(Guid caseId)
        {
            ...
            var result = await (from c in cases
                                select new CaseProgressReportDto
                                {
                                    ...
                                    HistoryProgress = _dbContext.CaseHistories.Include(x => x.FromEmployee).Include(x => x.ToEmployee).Where(x => x.CaseId == caseId && x.ReciverType == ReciverType.Orginal).Select(y => new CaseProgressReportHistoryDto
                                    {
        ...
        public static string getElapsedTime(DateTime? historyCreatedTime, DateTime? actionTime)
        {
            if (historyCreatedTime.HasValue && actionTime.HasValue)
            {
                DateTime start = historyCreatedTime.Value;
                DateTime end = actionTime.Value;

                // If actionTime is earlier than historyCreatedTime, return 0
                if (end < start) return "0 Min.";

                // Define work hours
                int workStartHour = 2;  // 2 AM
                int workEndHour = 23;   // 11 PM
                int lunchStartHour = 12; // 12 PM
                int lunchEndHour = 13;  // 1 PM

                // Helper function to check if it's a workday (Monday to Friday)
                bool IsWorkday(DateTime date)
                {
                    return date.DayOfWeek >= DayOfWeek.Monday && date.DayOfWeek <= DayOfWeek.Friday;
                }

                // Helper function to calculate working minutes for a single day
                int CalculateWorkingMinutesForDay(DateTime dayStart, DateTime dayEnd)
                {
                    // If both times are outside working hours, return 0
                    if (dayEnd.Hour < workStartHour || dayStart.Hour >= workEndHour)
                    {
                        return 0;
                    }

                    // Clamp the start and end times to fall within working hours
                    DateTime adjustedStart = dayStart;
                    DateTime adjustedEnd = dayEnd;

                    if (adjustedStart.Hour < workStartHour)
                        adjustedStart = adjustedStart.Date.AddHours(workStartHour);

                    if (adjustedEnd.Hour >= workEndHour)
                        adjustedEnd = adjustedEnd.Date.AddHours(workEndHour);

                    // Calculate minutes worked
                    int totalMinutes = 0;

                    // Exclude lunch break if applicable
                    if (adjustedStart.Hour < lunchStartHour && adjustedEnd.Hour > lunchEndHour)
                    {
                        // Calculate pre-lunch and post-lunch work minutes
                        totalMinutes = (lunchStartHour - adjustedStart.Hour) * 60 + (adjustedEnd.Hour - lunchEndHour) * 60;
                    }
                    else if (adjustedEnd.Hour <= lunchStartHour || adjustedStart.Hour >= lunchEndHour)
                    {
                        // No lunch break overlap, simple difference
                        totalMinutes = (int)(adjustedEnd - adjustedStart).TotalMinutes;
                    }

                    return totalMinutes;
                }

                // Main loop to calculate working minutes between the two dates
                ...
                while (current.Date <= end.Date)
                {
                    ...
                    {
                        DateTime currentDayEnd = current.Date.AddHours(workEndHour); // End of the working day (11 PM)

                        // If this is the last day, calculate minutes up to 'end' time
                        if (current.Date == end.Date)
                        {
                            totalWorkingMinutes += CalculateWorkingMinutesForDay(current, end);
                        }
                        else
                        {
                            // Calculate working minutes for the full day
                            totalWorkingMinutes += CalculateWorkingMinutesForDay(current, currentDayEnd);
                        }
                    }

                    // Move to the start of the next day (2 AM next working day)
                    current = current.AddDays(1).Date.AddHours(workStartHour);
                }

                // Convert total minutes into days, hours, and minutes
                int days = totalWorkingMinutes / 1440; // 1440 minutes in a day
                int remainingMinutesAfterDays = totalWorkingMinutes % 1440;
                ...
                if (hours > 0) result += (result == "" ? "" : ", ") + $"{hours} Hr.";
