    ...
    public class CaseTypeService : ICaseTypeService
    {
        ...
        public async Task Add(CaseTypePostDto caseTypeDto)
        {
            try
            {
                CaseType caseType = new()
                {
                    ...
                    OrganizationalStructureId = caseTypeDto.OrganizationalStructureId
                };
        ...
        public async Task UpdateCaseType(CaseTypePostDto caseTypeDto)
        {
            try
            {
                ...
                caseType.OrganizationalStructureId = caseTypeDto.OrganizationalStructureId;
        ...
        public async Task<List<CaseTypeGetDto>> GetAll(Guid subOrgId)
        {
            try
            {
                List<CaseType> caseTypes = await _dbContext.CaseTypes.Include(p => p.ParentCaseType).Include(p => p.OrganizationalStructure).Where(x => x.ParentCaseTypeId == null && x.SubsidiaryOrganizationId == subOrgId).ToListAsync();
                ...
                foreach (CaseType caseType in caseTypes)
                {
                    result.Add(new CaseTypeGetDto
                    {
                        ...
                        OrganizationalStructureId = caseType.OrganizationalStructureId,
                        OrganizationalStructureName = caseType.OrganizationalStructure?.StructureName,
                        OrganizationBranchId = caseType.OrganizationalStructure?.OrganizationBranchId,
        ...
        public async Task<List<SelectListDto>> GetAllByCaseForm(string caseForm, Guid subOrgId)
        {
            try
            {
                List<CaseType> caseTypes = await _dbContext.CaseTypes.Include(p => p.ParentCaseType).Include(p => p.OrganizationalStructure).Where(x => x.CaseForm == Enum.Parse<CaseForm>(caseForm) && x.ParentCaseTypeId == null && x.SubsidiaryOrganizationId == subOrgId).ToListAsync();
                ...
                foreach (CaseType caseType in caseTypes)
                {
                    result.Add(new SelectListDto
                    {
                        ...
                        Photo = caseType.OrganizationalStructure?.StructureName
        ...
        public async Task<List<SelectListDto>> GetAllSelectList(Guid subOrgId)
        {
            ...
            return await (from c in _dbContext.CaseTypes.Where(x => x.SubsidiaryOrganizationId == subOrgId)
                          select new SelectListDto
                          {
                              ...
                              Photo = c.OrganizationalStructure.StructureName
...
;
        ...
        {
            var children = await _dbContext.CaseTypes.Where(x => x.ParentCaseTypeId == caseTypeId).OrderBy(x => x.OrderNumber).Select(y => new CaseTypeGetDto
            {
                Id = y.Id,
                CaseTypeTitle = y.CaseTypeTitle,
                Code = y.Code,
