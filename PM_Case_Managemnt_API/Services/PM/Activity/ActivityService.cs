...
namespace PM_Case_Managemnt_API.Services.PM.Activity
{
    public class ActivityService : IActivityService
    {
        ...
        public async Task<List<ActivityViewDto>> GetActivtiesForApproval(Guid employeeId)
        {
            ...
            try
            {
                var employeePostion = await _dBContext.Employees.Where(x => x.Id == employeeId).Select(x => new { x.Position, x.OrganizationalStructureId }).FirstOrDefaultAsync();

                var not = (from p in _dBContext.Plans.Where(x => x.FinanceId == employeeId || x.ProjectManagerId == employeeId || (employeePostion.Position == Position.Director && employeePostion.OrganizationalStructureId == x.StructureId))
                           ...
                           select new
                           {
                               ...
                               ap.Id,
                           }).Union(from p in _dBContext.Plans.Where(x => x.FinanceId == employeeId || x.ProjectManagerId == employeeId || (employeePostion.Position == Position.Director && employeePostion.OrganizationalStructureId == x.StructureId))
                ...
                foreach (var activitprogress in not)
                {
                    ...
                    var activityViewDtos = (from e in _dBContext.ActivityProgresses.Include(x => x.Activity.ActivityParent.Task.Plan.Structure).Where(a => a.Id == activitprogress.Id && (a.IsApprovedByManager == approvalStatus.pending || a.IsApprovedByDirector == approvalStatus.pending || a.IsApprovedByFinance == approvalStatus.pending))
                                            ...
                                            select new ActivityViewDto
                                            {
                                                ...
                                                ProjectType = e.Activity.ActivityParentId != null
                                                            ? e.Activity.ActivityParent.Task.Plan.ProjectType
                                                            : (e.Activity.TaskId != null
                                                                ? e.Activity.Task.Plan.ProjectType
                                                                : e.Activity.Plan.ProjectType)
                                            }
                                       ).ToList();
