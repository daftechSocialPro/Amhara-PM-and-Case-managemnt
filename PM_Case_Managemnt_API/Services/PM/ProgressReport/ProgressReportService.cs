...
namespace PM_Case_Managemnt_API.Services.PM.ProgressReport
{
    public class ProgressReportService : IProgressReportService
    {
        ...
        public async Task<List<DiagramDto>> GetDirectorLevelPerformance(Guid subOrgId, Guid? branchId)
        {
            try
            {
                // Load all data in parallel and await them
                var structures = await LoadOrganizationalStructures(subOrgId);
                var employees = await LoadEmployees(subOrgId);
                var budgetYears = await LoadActiveBudgetYears(subOrgId);

                // Await all tasks
                //await System.Threading.Tasks.Task.WhenAll(structuresTask, employeesTask, budgetYearsTask);

                // Get the results
                //var structures = await structuresTask;
                //var employees = await employeesTask;
                //var budgetYears = await budgetYearsTask;


                // Get root structure and validate
                var rootStructure = structures.FirstOrDefault(x => x.ParentStructureId == null);
                if (rootStructure == null)
                {

                    return new List<DiagramDto>();
                }

                // Create hierarchical diagram
                var diagram = CreateDiagramHierarchy(structures, employees, budgetYears, rootStructure);

                return new List<DiagramDto> { diagram };
            }
            catch (Exception ex)
            {
                throw;
        ...
        private async Task<List<OrganizationalStructure>> LoadOrganizationalStructures(Guid subOrgId)
        {
            return await _dBContext.OrganizationalStructures
        ...
        private async Task<List<Employee>> LoadEmployees(Guid subOrgId)
        {
            //var structureIds = await _dBContext.OrganizationalStructures
            //    .Where(x => x.SubsidiaryOrganizationId == subOrgId)
            //    .Select(x => x.Id)
            //    .ToListAsync();

            return await _dBContext.Employees
                .Where(x => x.OrganizationalStructure.SubsidiaryOrganizationId == subOrgId && x.Position == Position.Director)
                .ToListAsync();
        }

        private async Task<List<Guid>> LoadActiveBudgetYears(Guid subOrgId)
        {
            return await _dBContext.ProgramBudgetYears
                .Where(x => x.SubsidiaryOrganizationId == subOrgId)
                .Join(
                    _dBContext.BudgetYears.Where(b => b.RowStatus == RowStatus.Active),
                    p => p.Id,
                    b => b.ProgramBudgetYearId,
                    (p, b) => b.Id
                )
                .ToListAsync();
        }

        private DiagramDto CreateDiagramHierarchy(
            List<OrganizationalStructure> structures,
            List<Employee> employees,
            List<Guid> budgetYears,
            OrganizationalStructure rootStructure)
        {
            var structureMap = new Dictionary<Guid, DiagramDto>();

            // Create all nodes first
            foreach (var structure in structures)
            {
                var contribution = GetContribution(structure.Id, structure.Weight, budgetYears);
                var director = employees.FirstOrDefault(x => x.OrganizationalStructureId == structure.Id);

                var node = new DiagramDto
                {
                    data = new
                    {
                        name = structure.StructureName,
                        weight = contribution,
                        head = director != null ? $"{director.Title} {director.FullName}" : string.Empty
                    },
                    ...
                    id = structure.Id,
                    parentId = structure.ParentStructureId,
                    order = structure.Order,
                    children = new List<DiagramDto>()
                };

                structureMap[structure.Id] = node;
            }

            // Build hierarchy
            foreach (var structure in structures.Where(x => x.ParentStructureId != null))
            {
                if (structureMap.TryGetValue(structure.ParentStructureId.Value, out var parent))
                {
                    parent.children.Add(structureMap[structure.Id]);
                }
            }

            return structureMap[rootStructure.Id];
        }
        ...
        public async Task<PlanReportByProgramDto> PlanReportByProgram(Guid subOrgId, string budgetYear, string reportBy)
        {
            try
            {
                ...
                {
                    return new PlanReportByProgramDto();
                }

                var programs = await GetProgramsForBudgetYear(subOrgId, budgetYear);
                var programViewModels = await BuildProgramViewModels(programs);
                var monthDeclarator = GetMonthDeclarator(programViewModels);

                if (monthDeclarator.Any())
                {
                    UpdateMonthNames(programViewModels, monthDeclarator, reportBy);
                }

                return new PlanReportByProgramDto
                {
                    MonthCounts = monthDeclarator,
        ...
        private async Task<List<Programs>> GetProgramsForBudgetYear(Guid subOrgId, string budgetYear)
        {
            int budgetYearPlan = Convert.ToInt32(budgetYear);
            var budgetYearIds = await _dBContext.BudgetYears
                .Where(x => x.Year == budgetYearPlan)
                .Select(x => x.ProgramBudgetYearId)
                .ToListAsync();

            return await _dBContext.Programs
                .Where(x => budgetYearIds.Contains(x.ProgramBudgetYearId) &&
                       x.SubsidiaryOrganizationId == subOrgId)
                .OrderBy(x => x.CreatedAt)
                .ToListAsync();
        }

        private async Task<List<ProgramViewModel>> BuildProgramViewModels(List<Programs> programs)
        {
            ...
            foreach (var program in programs)
            {
                var programViewModel = new ProgramViewModel
                {
                    ...
                    ProgramPlanViewModels = await BuildProgramPlanViewModels(program)
                };
        ...
        private async Task<List<ProgramPlanViewModel>> BuildProgramPlanViewModels(Programs program)
        {
            ...
            var planViewModels = new List<ProgramPlanViewModel>();

            foreach (var plan in plans)
            {
                var planViewModel = plan.HasTask ?
                    await BuildTaskBasedPlanViewModel(plan, program) :
                    await BuildActivityBasedPlanViewModel(plan, program);
        ...
        private async Task<ProgramPlanViewModel> BuildActivityBasedPlanViewModel(Models.PM.Plan plan, Programs program)
        {
            var activity = await _dBContext.Activities
                .Include(x => x.UnitOfMeasurement)
                .SingleOrDefaultAsync(x => x.PlanId == plan.Id);

            if (activity == null) return null;

            var targetActivities = await _dBContext.ActivityTargetDivisions
                .Where(x => x.ActivityId == activity.Id)
                .OrderBy(x => x.Order)
                .ToListAsync();

            return new ProgramPlanViewModel
            {
                ProgramName = program.ProgramName,
                PlanNAme = plan.PlanName,
                MeasurementUnit = activity.UnitOfMeasurement.Name,
                TotalBirr = plan.PlandBudget,
                TotalGoal = plan.Activities.Sum(x => x.Goal),
                FiscalPlanPrograms = targetActivities.Select(t => new FiscalPlanProgram
                {
                    RowORder = t.Order,
                    FinancialValue = t.TargetBudget,
                    fisicalValue = t.Target
                }).ToList()
            };
        }

        private async Task<ProgramPlanViewModel> BuildTaskBasedPlanViewModel(Models.PM.Plan plan, Programs program)
        {
            ...
            var fiscalPrograms = new List<FiscalPlanProgram>();
            float totalBirr = 0;
            ...
            foreach (var task in tasks)
            {
                ...
                {
                    var totals = await ProcessSimpleTask(task, fiscalPrograms);
                    totalBirr += totals.TotalBirr;
                    if (string.IsNullOrEmpty(measurementName))
                    {
                        measurementName = totals.MeasurementName;
                    }
                }
                else
                {
                    totalBirr += await ProcessComplexTask(task, fiscalPrograms);
                }
            }
            ...
            return new ProgramPlanViewModel
            {
                ProgramName = program.ProgramName,
                PlanNAme = plan.PlanName,
                MeasurementUnit = measurementName,
                TotalBirr = totalBirr,
                TotalGoal = fiscalPrograms.Sum(x => x.fisicalValue),
                FiscalPlanPrograms = fiscalPrograms
            };
        }
        ...
        private async Task<TaskTotals> ProcessSimpleTask(Models.PM.Task task, List<FiscalPlanProgram> fiscalPrograms)
        {
            var totals = new TaskTotals();

            var activity = await _dBContext.Activities
                .Include(x => x.UnitOfMeasurement)
                .FirstOrDefaultAsync(x => x.TaskId == task.Id);

            if (activity == null) return totals;

            totals.TotalBirr = task.PlanedBudget;
            totals.MeasurementName = activity.UnitOfMeasurement.Name;

            await ProcessActivityTargets(activity, fiscalPrograms);

            return totals;
        }

        private async Task<float> ProcessComplexTask(Models.PM.Task task, List<FiscalPlanProgram> fiscalPrograms)
        {
            var parentActivities = await _dBContext.ActivityParents
                .Include(x => x.Activities)
                .Where(x => x.TaskId == task.Id)
                .ToListAsync();

            float totalBirr = parentActivities.Sum(x => x.PlanedBudget);

            foreach (var parentActivity in parentActivities)
            {
                foreach (var subActivity in parentActivity.Activities)
                {
                    await ProcessActivityTargets(subActivity, fiscalPrograms, true);
                }
            }
        ...
        private async System.Threading.Tasks.Task ProcessActivityTargets(Models.PM.Activity activity, List<FiscalPlanProgram> fiscalPrograms,
        bool isComplex = false)
        {
            var query = _dBContext.ActivityTargetDivisions
                .Include(x => x.Activity);

            if (isComplex)
            {
                query = (Microsoft.EntityFrameworkCore.Query.IIncludableQueryable<ActivityTargetDivision, Models.PM.Activity?>)query.Include(x => x.Activity)
                            .ThenInclude(a => a.ActivityParent)
                            .ThenInclude(ap => ap.Task);
            }
            else
            {
                query = (Microsoft.EntityFrameworkCore.Query.IIncludableQueryable<ActivityTargetDivision, Models.PM.Activity?>)query.Include(x => x.Activity)
            ...
            {
                await UpdateFiscalProgram(target, fiscalPrograms, isComplex);
            }
        }

        private async System.Threading.Tasks.Task UpdateFiscalProgram(ActivityTargetDivision target,
            List<FiscalPlanProgram> fiscalPrograms, bool isComplex)
        {
            var existingProgram = fiscalPrograms.FirstOrDefault(x => x.RowORder == target.Order);
            float physicalValue;

            if (isComplex)
            {
                physicalValue = (target.Target * target.Activity.Weight) /
                    (float)target.Activity.ActivityParent.Task.Weight;
            }
            else
            {
                var totalWeight = await _dBContext.Activities
                    .Where(x => x.TaskId == target.Activity.TaskId)
                    .SumAsync(x => x.Weight);
                physicalValue = (target.Target * target.Activity.Weight) / totalWeight;
            }

            if (existingProgram == null)
            {
                fiscalPrograms.Add(new FiscalPlanProgram
                {
                    RowORder = target.Order,
                    FinancialValue = target.TargetBudget,
                    fisicalValue = (uint)Math.Round(physicalValue, 2)
                });
            }
            else
            {
                existingProgram.FinancialValue += target.TargetBudget;
                existingProgram.fisicalValue += (uint)Math.Round(physicalValue, 2);
            }
        }

        private List<FiscalPlanProgram> GetMonthDeclarator(List<ProgramViewModel> programViewModels)
        {
            return programViewModels
                .Where(x => x.ProgramPlanViewModels.Any())
                .FirstOrDefault()
                ?.ProgramPlanViewModels
                .FirstOrDefault(x => x.FiscalPlanPrograms.Any())
                ?.FiscalPlanPrograms
                .ToList() ?? new List<FiscalPlanProgram>();
        }

        private void UpdateMonthNames(List<ProgramViewModel> programViewModels,
            List<FiscalPlanProgram> monthDeclarator, string reportBy)
        {
            if (reportBy == "Quarter")
            {
                ProcessQuarterlyReport(programViewModels);
            }
            else
            {
                ProcessMonthlyReport(monthDeclarator);
            }
        }

        private void ProcessQuarterlyReport(List<ProgramViewModel> programViewModels)
        {
            for (int quarter = 0; quarter < 4; quarter++)
            {
                int monthsInQuarter = (quarter + 1) * 3;

                foreach (var program in programViewModels)
                {
                    foreach (var plan in program.ProgramPlanViewModels)
                    {
                        var monthsInCurrentQuarter = plan.FiscalPlanPrograms
                            .Where(x => x.RowORder <= monthsInQuarter && x.MonthName == null)
                            .ToList();

                        if (!monthsInCurrentQuarter.Any()) continue;

                        var quarterSummary = new FiscalPlanProgram
                        {
                            RowORder = quarter + 1,
                            MonthName = $"Quarter {quarter + 1}",
                            fisicalValue = monthsInCurrentQuarter.Sum(x => x.fisicalValue),
                            FinancialValue = monthsInCurrentQuarter.Sum(x => x.FinancialValue)
                        };

                        plan.FiscalPlanPrograms.Add(quarterSummary);
                        plan.FiscalPlanPrograms.RemoveRange(0, monthsInCurrentQuarter.Count);
                    }
                }
            }
        }

        private void ProcessMonthlyReport(List<FiscalPlanProgram> monthDeclarator)
        {
            var dateTimeFormatInfo = new System.Globalization.DateTimeFormatInfo();

            for (int i = 0; i < monthDeclarator.Count; i++)
            {
                int monthNumber = i >= 6 ? i - 5 : i + 7;
                monthDeclarator[i].MonthName = dateTimeFormatInfo.GetMonthName(monthNumber);
            }
        }
        ...
        {
            var planReportDetailDto = new PlanReportDetailDto();
            try
            {
                if (BudgetYear == null) return planReportDetailDto;

                var monthDeclarator = new List<ActivityTargetDivisionReport>();
                var programWithStructure = await GetProgramStructures(ProgramId);

                if (monthDeclarator.Any())
                {
                    ProcessReportingPeriods(ReportBy, monthDeclarator, programWithStructure);
                }
        ...
        private async Task<List<ProgramWithStructure>> GetProgramStructures(string programId)
        {
            var programWithStructure = new List<ProgramWithStructure>();
            var newProgram = Guid.Parse(programId);
            var structuresInProgram = await _dBContext.Plans
                .Where(x => x.ProgramId == newProgram)
                .Select(x => x.StructureId)
                .Distinct()
                .ToListAsync();

            var structures = await _dBContext.OrganizationalStructures
                .Where(x => structuresInProgram.Contains(x.Id))
                .ToListAsync();

            foreach (var structure in structures)
            {
                var programStructure = new ProgramWithStructure
                {
                    StrutureName = structure.StructureName,
                    StructurePlans = await GetStructurePlans(structure.Id, newProgram)
                };
                programWithStructure.Add(programStructure);
            }

            return programWithStructure;
        }

        private async Task<List<StructurePlan>> GetStructurePlans(Guid structureId, Guid programId)
        {
            var plansInStructure = await _dBContext.Plans
                .Include(x => x.Tasks)
                    .ThenInclude(x => x.ActivitiesParents)
                ...
                .Include(x => x.Tasks)
                    .ThenInclude(x => x.ActivitiesParents)
                        .ThenInclude(x => x.Activities)
                            .ThenInclude(x => x.ActivityTargetDivisions)
                .Include(x => x.Tasks)
                    .ThenInclude(x => x.Activities)
                        .ThenInclude(x => x.ActivityTargetDivisions)
                .Include(x => x.Activities)
            ...
            return plansInStructure.Select(plan => CreateStructurePlan(plan)).ToList();
        }

        private StructurePlan CreateStructurePlan(Models.PM.Plan plan)
        {
            ...
            if (plan.HasTask)
            {
                structurePlan.PlanTasks = CreatePlanTasks(plan.Tasks.OrderBy(x => x.CreatedAt));
            }
            else
            {
                structurePlan.PlanTargetDivision = CreatePlanTargetDivision(plan.Activities.FirstOrDefault());
                if (plan.Activities.FirstOrDefault() is { } activity)
                {
                    structurePlan.Weight = activity.Weight;
                    structurePlan.UnitOfMeasurement = activity.UnitOfMeasurement.Name;
                    structurePlan.Target = activity.Goal;
                }
            }
        ...
        private List<PlanTask> CreatePlanTasks(IEnumerable<Models.PM.Task> tasks)
        {
            ...
            foreach (var task in tasks)
            {
                ...
                if (task.HasActivityParent)
                {
                    planTask.TaskActivities = CreateTaskActivities(task.ActivitiesParents.OrderBy(x => x.CreatedAt));
                }
                else
                {
                    planTask.TaskTargetDivision = CreateTaskTargetDivision(task.Activities.FirstOrDefault());
                    if (task.Activities.FirstOrDefault() is { } activity)
                    {
                        planTask.Weight = activity.Weight;
                        planTask.UnitOfMeasurement = activity.UnitOfMeasurement.Name;
                        planTask.Target = activity.Goal;
                    }
                }
        ...
        private List<TaskActivity> CreateTaskActivities(IEnumerable<ActivityParent> activityParents)
        {
            var taskActivities = new List<TaskActivity>();
            foreach (var parent in activityParents)
            {
                var taskActivity = new TaskActivity
                {
                    ActivityName = parent.ActivityParentDescription
                };

                if (parent.HasActivity)
                {
                    taskActivity.ActSubActivity = CreateSubActivities(parent.Activities.OrderBy(x => x.CreatedAt));
                }
                else
                {
                    taskActivity.ActivityTargetDivision = CreateActivityTargetDivision(parent.Activities.FirstOrDefault());
                    if (parent.Activities.FirstOrDefault() is { } activity)
                    {
                        taskActivity.Weight = activity.Weight;
                        taskActivity.UnitOfMeasurement = activity.UnitOfMeasurement.Name;
                        taskActivity.Target = activity.Goal;
                    }
                }
        ...
        private List<ActSubActivity> CreateSubActivities(IEnumerable<Models.PM.Activity> activities)
        {
            return activities.Select(activity => new ActSubActivity
            {
                ...
                subActivityTargetDivision = CreateSubActivityTargetDivision(activity)
            }).ToList();
        }

        private List<ActivityTargetDivisionReport> CreateSubActivityTargetDivision(Models.PM.Activity activity)
        {
            return activity.ActivityTargetDivisions.Select(division => new ActivityTargetDivisionReport
            {
                Order = division.Order,
                TargetValue = division.Target
            }).ToList();
        }

        private void ProcessReportingPeriods(string reportBy, List<ActivityTargetDivisionReport> monthDeclarator, List<ProgramWithStructure> programWithStructure)
        {
            if (reportBy == "Quarter")
            {
                ProcessQuarterlyReport(monthDeclarator, programWithStructure);
            }
            else
            {
                ProcessMonthlyReport(monthDeclarator);
        ...
        private void ProcessQuarterlyReport(List<ActivityTargetDivisionReport> monthDeclarator, List<ProgramWithStructure> programWithStructure)
        {
            monthDeclarator.Clear();
            for (int i = 0; i < 4; i++)
            {
                int endMonth = (i + 1) * 3;
                foreach (var structure in programWithStructure)
                {
                    ProcessQuarterlyStructure(structure, i, endMonth, monthDeclarator);
                }
            }
        }

        private void ProcessQuarterlyStructure(ProgramWithStructure structure, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            foreach (var plan in structure.StructurePlans)
            {
                if (plan.PlanTargetDivision != null)
                {
                    ProcessQuarterlyPlanTargets(plan, quarterIndex, endMonth, monthDeclarator);
                }
                else if (plan.PlanTasks != null)
                {
                    foreach (var task in plan.PlanTasks)
                    {
                        ProcessQuarterlyTaskTargets(task, quarterIndex, endMonth, monthDeclarator);
                    }
                }
            }
        }

        private void ProcessMonthlyReport(List<ActivityTargetDivisionReport> monthDeclarator)
        {
            for (int i = 0; i < monthDeclarator.Count; i++)
            {
                int monthNumber = i >= 6 ? i - 5 : i + 7;
        ...
        private List<ActivityTargetDivisionReport> CreatePlanTargetDivision(Models.PM.Activity activity)
        {
            if (activity == null) return new List<ActivityTargetDivisionReport>();

            return activity.ActivityTargetDivisions.Select(tp => new ActivityTargetDivisionReport
            {
                Order = tp.Order,
                TargetValue = tp.Target
            }).ToList();
        }

        private List<ActivityTargetDivisionReport> CreateTaskTargetDivision(Models.PM.Activity activity)
        {
            return CreatePlanTargetDivision(activity);
        }

        private List<ActivityTargetDivisionReport> CreateActivityTargetDivision(Models.PM.Activity activity)
        {
            return CreatePlanTargetDivision(activity);
        }
        private void ProcessQuarterlyPlanTargets(StructurePlan plan, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            var newMonth = plan.PlanTargetDivision.Where(x => x.Order <= endMonth && x.MonthName == null).ToList();
            if (newMonth.Any())
            {
                var quarterReport = new ActivityTargetDivisionReport
                {
                    Order = quarterIndex + 1,
                    MonthName = $"Quarter {quarterIndex + 1}",
                    TargetValue = newMonth.Sum(x => x.TargetValue)
                };

                plan.PlanTargetDivision.Add(quarterReport);
                plan.PlanTargetDivision.RemoveRange(0, newMonth.Count);
        ...
        private void ProcessQuarterlyTaskTargets(PlanTask task, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            if (task.TaskTargetDivision != null)
            {
                ProcessQuarterlyTaskTargetDivision(task, quarterIndex, endMonth, monthDeclarator);
            }
            else if (task.TaskActivities != null)
            {
                foreach (var activity in task.TaskActivities)
                {
                    ProcessQuarterlyActivityTargets(activity, quarterIndex, endMonth, monthDeclarator);
                }
            }
        }

        private void ProcessQuarterlyTaskTargetDivision(PlanTask task, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            var newMonthTask = task.TaskTargetDivision.Where(x => x.Order <= endMonth && x.MonthName == null).ToList();
            if (newMonthTask.Any())
            {
                var quarterReport = new ActivityTargetDivisionReport
                {
                    Order = quarterIndex + 1,
                    MonthName = $"Quarter {quarterIndex + 1}",
                    TargetValue = newMonthTask.Sum(x => x.TargetValue)
                };

                task.TaskTargetDivision.Add(quarterReport);
        ...
        private void ProcessQuarterlyActivityTargets(TaskActivity activity, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            if (activity.ActivityTargetDivision != null)
            {
                ProcessQuarterlyActivityTargetDivision(activity, quarterIndex, endMonth, monthDeclarator);
            }
            else if (activity.ActSubActivity != null)
            {
                foreach (var subActivity in activity.ActSubActivity)
                {
                    ProcessQuarterlySubActivityTargets(subActivity, quarterIndex, endMonth, monthDeclarator);
                }
            }
        }

        private void ProcessQuarterlyActivityTargetDivision(TaskActivity activity, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            var newMonthAct = activity.ActivityTargetDivision.Where(x => x.Order <= endMonth && x.MonthName == null).ToList();
            if (newMonthAct.Any())
            {
                var quarterReport = new ActivityTargetDivisionReport
                {
                    Order = quarterIndex + 1,
                    MonthName = $"Quarter {quarterIndex + 1}",
                    TargetValue = newMonthAct.Sum(x => x.TargetValue)
                };

                activity.ActivityTargetDivision.Add(quarterReport);
                activity.ActivityTargetDivision.RemoveRange(0, newMonthAct.Count);
        ...
        private void ProcessQuarterlySubActivityTargets(ActSubActivity subActivity, int quarterIndex, int endMonth, List<ActivityTargetDivisionReport> monthDeclarator)
        {
            if (subActivity.subActivityTargetDivision != null)
            {
                var newMonthSAct = subActivity.subActivityTargetDivision.Where(x => x.Order <= endMonth && x.MonthName == null).ToList();
                if (newMonthSAct.Any())
                {
                    var quarterReport = new ActivityTargetDivisionReport
                    {
                        Order = quarterIndex + 1,
                        MonthName = $"Quarter {quarterIndex + 1}",
                        TargetValue = newMonthSAct.Sum(x => x.TargetValue)
                    };

                    subActivity.subActivityTargetDivision.Add(quarterReport);
                    subActivity.subActivityTargetDivision.RemoveRange(0, newMonthSAct.Count);
        ...
        //    var orgStructures = await _dBContext.OrganizationalStructures.Where(x => x.SubsidiaryOrganizationId == subOrgId).Include(x => x.ParentStructure).ToListAsync();
        //    var orgStructureIds = orgStructures.Select(x => x.Id).ToList();

        //    var employess = await _dBContext.Employees.Where(x => orgStructureIds.Contains(x.OrganizationalStructureId)).ToListAsync();
        //    var childs = new List<DiagramDto>();

        //    var parentStructure = orgStructures.FirstOrDefault(x => x.ParentStructureId == null);

        //    var programBudgetYearsIds = await _dBContext.ProgramBudgetYears.Where(x => x.SubsidiaryOrganizationId == subOrgId).Select(x => x.Id).ToListAsync();

        //    var BudgetYear = await _dBContext.BudgetYears.Where(x => programBudgetYearsIds.Contains(x.ProgramBudgetYearId) && x.RowStatus == RowStatus.Active).Select(x => x.Id).ToListAsync();
        ...
        //            weight = GetContribution(parentStructure.Id, parentStructure.Weight, BudgetYear),
        ...
        //        expanded = true,
        //        type = "organization",
        //        styleClass = GetContribution(parentStructure.Id, parentStructure.Weight, BudgetYear) < 25.00 ? "bg-danger text-white" : "bg-success text-white",
        ...
        //                weight = GetContribution(x.Id, x.Weight, BudgetYear),
        ...
        //            expanded = true,
        //            type = "organization",
        //            styleClass = GetContribution(x.Id, x.Weight, BudgetYear) < 25.00 ? "bg-danger text-white" : "bg-success text-white",
        //            id = x.Id,
        ...
        //            int budgetYearPlan = Convert.ToInt32(BudgetYear);
        //            var BudgetYearValue = await _dBContext.BudgetYears.Where(x => x.Year == budgetYearPlan).Select(x => x.ProgramBudgetYearId).ToListAsync();
        //            var ProgLists = await _dBContext.Programs.Where(x => BudgetYearValue.Contains(x.ProgramBudgetYearId) && x.SubsidiaryOrganizationId == subOrgId).OrderBy(x => x.CreatedAt).ToListAsync();
        ...
        //                var plansLst = await _dBContext.Plans.Where(x => x.ProgramId == items.Id).OrderBy(x => x.CreatedAt).ToListAsync();
        ...
        //                    {
        //                        var PlanActivities = await _dBContext.Activities.Include(x => x.UnitOfMeasurement).SingleOrDefaultAsync(x => x.PlanId == planItems.Id);
        ...
        //                        var TasksOnPlan = await _dBContext.Tasks.Where(x => x.PlanId == planItems.Id).OrderBy(x => x.CreatedAt).ToListAsync();
        ...
        //                                    {
        //                                        var TargetForTasks = await _dBContext.ActivityTargetDivisions.Include(x => x.Activity).Include(x => x.Activity).ThenInclude(x => x.ActivityParent).Where(x => x.ActivityId == SubAct.Id).OrderBy(x => x.Order).ToListAsync();
        //                                        foreach (var tarItems in TargetForTasks)
        //                                        {
        //                                            if (!fsForPlan.Where(x => x.RowORder == tarItems.Order).Any())
        //                                            {
        ...
        //            var StrucsinProg = await _dBContext.Plans.Where(x => x.ProgramId == newProgram).Select(x => x.StructureId).Distinct().ToListAsync();
        //            var Structures = await _dBContext.OrganizationalStructures.Where(x => StrucsinProg.Contains(x.Id)).ToListAsync();
        ...
        public async Task<PlannedReport> PlanReports(string BudgetYea, Guid selectStructureId, string ReportBy)
        {
            ...
            try
            {
                ...
                var BudgetYear = await _dBContext.BudgetYears.Where(x => x.Year.ToString() == BudgetYea).Select(x => x.Id).ToListAsync();
                ...
                var allPlans = _dBContext.Plans
                     ...
                     .Where(x => x.StructureId == selectStructureId && BudgetYear.Contains((Guid)x.BudgetYearId)).OrderBy(c => c.CreatedAt).ToList();
        ...
        public async Task<ProgresseReport> ProgresssReport(FilterationCriteria filterationCriteria)
        {
            ...
            try
            {
                ...
                if (filterationCriteria.actParentId != Guid.Empty || filterationCriteria.actParentId != null)
                {
                    List<QuarterMonth> QuarterMonth = new List<QuarterMonth>();
                ...
                if (filterationCriteria.taskId != Guid.Empty)
                {
                    ...
                    else if (task.Activities.Any() /*&& task.Activities.FirstOrDefault().ActivityTargetDivisions.Any()*/)
                    {
                        ...
                        actes.AddRange(task.Activities.ToList()/*.FirstOrDefault()*/);
        ...
        public float GetContribution(Guid structureId, float weight, List<Guid> budetyearIds)
        {
            float performance = 0;
            ...
            var Plans = _dBContext.Plans
                ...
                .Where(x => x.StructureId == structureId && budetyearIds.Contains((Guid)x.BudgetYearId)).ToList();
