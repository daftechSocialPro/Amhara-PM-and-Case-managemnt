...
namespace PM_Case_Managemnt_API.Services.Common.ShelfService
{
    public class ShelfService : IShelfService
    {
        ...
        public async Task Add(ShelfPostDto shelfPostDto)
        {
            try
            {
                Shelf newShelf = new()
                {
                    ...
                    OrganizationalStructureId = shelfPostDto.OrganizationalStructureId,
                };
        ...
        public async Task<List<ShelfGetDto>> GetAll(Guid subOrgId)
        {
            try
            {
                return (await _dbContext.Shelf.Where(x => x.OrganizationalStructureId == subOrgId).Select(x => new ShelfGetDto()
                {
                    Id = x.Id,
                    Remark = x.Remark,
