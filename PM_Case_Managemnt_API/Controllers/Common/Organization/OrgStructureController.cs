...
namespace PM_Case_Managemnt_API.Controllers.Common.Organization
{
    ...
    public class OrgStructureController : ControllerBase
    {
        ...
        public async Task<IActionResult> Update([FromBody] OrgStructureDto orgStructure)
        {
            try
            {
                var response = await _orgStructureService.UpdateOrganizationalStructure(orgStructure);
        ...
        [HttpDelete]
        public async Task<IActionResult> DeleteOrganizationalStructure(Guid organizationStructurId)
        {
            return Ok(await _orgStructureService.DeleteOrganizationalStructure(organizationStructurId));
        }
